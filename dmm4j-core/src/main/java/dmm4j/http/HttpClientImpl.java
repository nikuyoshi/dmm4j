...
class HttpClientImpl extends HttpClientBase implements HttpResponseCode, java.io.Serializable {
  ...
  static {
    ...
    try {
      ...
      if (Integer.parseInt((String) Class.forName("android.os.Build$VERSION").getField("SDK")
          .get(null)) < 8) {
  ...
  private static final Map<HttpClientConfiguration, HttpClient> instanceMap =
      new HashMap<HttpClientConfiguration, HttpClient>(1);
  ...
  public HttpResponse post(String url, HttpParameter[] params) throws DmmException {
    return request(new HttpRequest(RequestMethod.POST, url, params, null));
  }

  @Override
  public HttpResponse handleRequest(HttpRequest req) throws DmmException {
    int retriedCount;
    ...
    for (retriedCount = 0; retriedCount < retry; retriedCount++) {
      ...
      try {
        ...
        try {
          ...
          if (req.getMethod() == RequestMethod.POST) {
            if (HttpParameter.containsFile(req.getParameters())) {
              String boundary = "----dmm4j-upload" + System.currentTimeMillis();
              ...
              for (HttpParameter param : req.getParameters()) {
                if (param.isFile()) {
                  ...
                  BufferedInputStream in =
                      new BufferedInputStream(param.hasFileBody() ? param.getFileBody()
          ...
          if (responseCode < OK || (responseCode != FOUND && MULTIPLE_CHOICES <= responseCode)) {
            if (responseCode == ENHANCE_YOUR_CLAIM || responseCode == BAD_REQUEST
              ...
              throw new DmmException(res.asString(), res);
            }
        ...
        if (retriedCount == CONF.getHttpRetryCount()) {
          throw new DmmException(ioe.getMessage(), ioe, responseCode);
        }
      }
      try {
  ...
  HttpURLConnection getConnection(String url) throws IOException {
    ...
    if (isProxyConfigured()) {
      ...
      final Proxy proxy =
          new Proxy(Proxy.Type.HTTP, InetSocketAddress.createUnresolved(CONF.getHttpProxyHost(),
