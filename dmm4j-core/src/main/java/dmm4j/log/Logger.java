...
import dmm4j.conf.Configuration;
...
public abstract class Logger {
  ...
  private static final String LOGGER_FACTORY_IMPLEMENTATION = "dmm4j.loggerFactory";

  static {
    LoggerFactory loggerFactory = null;
    // -Ddmm4j.debug=true -Ddmm4j.loggerFactory=dmm4j.StdOutLoggerFactory
    ...
    // configuration in dmm4j.properties
    // loggerFactory=dmm4j.StdOutLoggerFactory
    ...
    if (null == loggerFactory) {
      loggerFactory =
          getLoggerFactoryIfAvailable("org.slf4j.impl.StaticLoggerBinder",
              "dmm4j.SLF4JLoggerFactory");
    }
    // otherwise, use commons-logging if it's found in the classpath
    if (null == loggerFactory) {
      loggerFactory =
          getLoggerFactoryIfAvailable("org.apache.commons.logging.Log",
              "dmm4j.CommonsLoggingLoggerFactory");
    }
    // otherwise, use log4j if it's found in the classpath
    if (null == loggerFactory) {
      loggerFactory =
          getLoggerFactoryIfAvailable("org.apache.log4j.Logger", "dmm4j.Log4JLoggerFactory");
    }
    // on Google App Engine, use java.dmm4j.JULogger
    if (null == loggerFactory) {
      loggerFactory =
          getLoggerFactoryIfAvailable("com.google.appengine.api.urlfetch.URLFetchService",
              "dmm4j.JULLoggerFactory");
    }
  ...
  private static LoggerFactory getLoggerFactoryIfAvailable(String checkClassName,
